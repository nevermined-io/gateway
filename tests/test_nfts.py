...
from contracts_lib_py.generic_contract import GenericContractExternal
...
def test_nft721_transfer(client, provider_account, consumer_account, publisher_account):
    ...
    # nft721 = GenericContractExternal(nft_address, transfer_abi_path, 'TransferNFT721Condition')
    #
    # if (nft721.address != nft_address):
    #     print('[KO] Keeper TransferNFT721Condition address {} ARE NOT the same NFT Address {}'. format(nft721.address, nft_address))
    # else:
    #     print('[OK] Keeper TransferNFT721Condition address {} == Address {}'. format(nft721.address, nft_address))
    #

    # keeper.nft721_upgradeable.add_minter(provider_account.address, provider_account)
    ...
    # keeper.nft721_upgradeable.set_approval_for_all(
    #     get_provider_account().address,
    #     True,
    #     publisher_account
    # )
    ...
    # time.sleep(10)
    #
    # is_approved = keeper.nft721_upgradeable.is_approved_for_all(
    #     publisher_account.address,
    #     provider_account.address
    # )
    # assert is_approved is True
    ...
    print('+++ agreement_id = {}'.format(agreement_id[1]))
    print('+++ asset_id = {}'.format(asset_id))
    print('+++ nft_holder_address = {}'.format(publisher_account.address))
    print('+++ nft_receiver_address = {}'.format(consumer_account.address))
    print('+++ nft_amount = {}'.format(nft_amounts))
    print('+++ lock_payment_condition_id = {}'.format(lock_payment_condition_id[1]))
    print('+++ cond_ids[0] - lock_payment_condition_id = {}'.format(cond_ids[0]))
    print('+++ transfer_nft = {}'.format(transfer_nft))
    print('+++ nft_contract_address = {}'.format(nft_address))
